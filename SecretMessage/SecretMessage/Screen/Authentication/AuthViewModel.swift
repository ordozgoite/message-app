...
extension AuthViewModel {
    func signInWithEmailPassword() async -> Bool {
        ...
        do {
            try await Auth.auth().signIn(withEmail: self.emailInput, password: self.passwordInput)
            return true
        }
        catch  {
            print(error)
    ...
    func signUpWithEmailPassword() async -> Bool {
        authenticationState = .authenticating
        do  {
            try await Auth.auth().createUser(withEmail: emailInput, password: passwordInput)
            return true
        }
        catch {
            print(error)
...
extension AuthViewModel {
    func postNewUser(token: String) async -> Bool {
        let result = await SCServices.shared.postNewUser(username: self.usernameInput, token: token)
    ...
    func getUserInfo(token: String) async -> Bool {
        let result = await SCServices.shared.getUserInfo(token: token)
